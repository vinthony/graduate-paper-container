...
随着信息时代的到来，搜索引擎作为互联网的入口成为人们网上冲浪的首选。而面对如今信息爆炸，提高搜索引擎的效率成为了从数以亿计的数据中获取到关键的信息的关键。而采用分布式网络爬虫技术提取和存储文档信息也成为了搜索引擎效率的关键。
 ...
 本文尝试使用一种改进式的主从结构的网络爬虫系统，在客户端中使用一个队列来对本节点解析后的数据进行过滤和提取，同时将不满足要求的链接发送到服务器中去。同时实验性的使用Redis作为优先级队列的容器，使用MongoDB来存储抓取到的网页信息。NoSQL的应用提高了整个系统的效率。应用一定的软件工程思想，将可配置项都提取出来，具有高度的可配置性。
 关键词： 网络爬虫；分布式；信息采集；NoSQL
### Abstract
Today we are living in a era of information, Search Engine is the first choice of the method of people who surf the Internet. When we facing the Information Explosion, the efficiency of Search Engine is the key of people gain information from digging millions of data and website. So the web crawler which support distributing calculation and distributing store becomes the key of Search Engine efficiency.
This article is about the design and the implementation of a distributing web crawler system and finishing the huge data collection step. the collection method of this web crawler system is full-website collection and save all information to the database.it will be convenience for the latter use.
This article try to use an improvement master-salver structure in the web crawler,we will have a queue in the client side to contain the parsed documents links which is suitable that client by DNS ,and on the other hand,using the HTTP
communication to send the rest of urls to server ,and server will distribute them in the future. Meanwhile, trying to use Redis as the container of priority queue of URL,and using MongoDB to store the information about fetched data.the usage of NoSQL improve the efficiency of whole system. This system also applies some theories of software Engineering to make the whole system has rich config setting.
...
##### 抓取模块
抓取模块通过制定的URL地址，到目标服务器上获取网络资源。是于互联网交互最频繁、占用网络带宽最多的功能模块。网络资源可以为html本文或者其他多媒体文件，网络爬虫发送HTTP请求到目标服务器后，根据服务器响应的服务器编码识别此次请求的结果。解析模块提取文本和网页的链接集合，将抓取模块抓取到的文档进行解析，抽取出文本和URL集合，并且对URL进行一些处理之后，添加到待抓取队列。
##### 重复消除模块
...
##### 分布式网络爬虫
随着互联网技术的发展以及风起云涌的云计算浪潮，爬虫技术也逐渐向着分布式的方向发展。虽然单机网络爬虫结构简单、方便维护、易于控制，但由于其硬件配置有限，无法完成大规模的分布式任务，而分布式技术不仅可以解决IT运营成本的问题，还可以解决爬虫的效率问题。
##### 分布式网络爬虫的现状
目前,最成功的分布式 Web 网络爬虫系统主要应用在搜索引擎公司(如: Google)和其他商业性较强的公司里。然而,这些公司并没有公布他们分布式 爬虫的技术细节并且云计算的应用尚处在萌芽阶段。现在比较著名的分布式网 络爬虫有 Mercator、UbiCrawler、WebFountain 和 Google Crawler。
Mercator分布式爬虫项目是一个例外,因为它的整个设计方案全部公布 于众了。这个分布式网络爬虫是由 Compaq 完成的,目前 AltaVista 搜索引擎正 在使用这个爬虫系统。而且 Google 在 1998 年最初也是用的这个爬虫系统。
UbiCrawler分布式爬虫项目是一个高性能、大规模的分布式网络爬虫, 这个爬虫主要特点是平台独立性即可以跨平台、大规模分布式性能良好、高容 错性、高效的分配函数(基于一致性哈希函数)和可自行调整爬行任务优先级。 另外该爬虫是由 Java 编写完成。
WebFountain分布式爬虫项目是由 IBM 开发完成的。WebFountain 与 Mercator 类似采用分布式模块化,但是使用 C++语言编写的。它的特点是一个 管理员机器控制一系列的蚂蚁机器。经过多次下载页面后,页面的变化率可以 推测出来。这时,一个非线性的方法必须用于求解方程以获得一个最大的新鲜 度的访问策略。
Google Crawler 爬虫系统使用成千上万台小型机和微机进行合作,完成分 布式抓取工作的。它使用一个 URLServer 分配 URL 给网络爬虫让他们去爬 取。网络爬虫爬取的网页再传送到一个 StoreSever 上去,由 StoreSever 将网页进行压缩并将其存储起来。
##### 本章小结
本章主要对爬虫系统中出现的一些概念做出了详细的解释，并对爬虫系统进行了完整的剖析，介绍了每个模块的主要功能。同时描述了分布式爬虫系统的概念和现状

### 第三章 分布式网络爬虫系统架构设计
#####分布式爬虫系统架构设计
常见的分布式网络爬虫系统架构主要分为三种：主从模式(master-slave)，点对点模式(Peer to Peer )和混合模式(Mixed)

主从模式是有一台硬件设施较好的机器作为主节点，负责整个种子url的分发和网络服务器的架构，其中也存储了各个分节点机器的使用效率以及正在进行的任务负载。同时通过DNS解析将种子队列的数据结构分发到适当的客户端节点中去，并接受客户端节点发送的待分配的解析数据到队列中，并进行进一步分配。该爬虫系统适合爬虫节点比较少的的系统，因为只有主节点有控制其他节点的能力和服务器的相关信息，从而避免了因客户端爬虫节点之间通信和协调之间产生的问题。但是该模式下，系统的扩展性并不好。负载压力都集中在了主节点上，当爬虫节点数量增多时，主节点的效率将会成为整个爬虫的瓶颈

点对点的模式下，爬虫节点与节点之间是相互独立制约的，和主从模式不同的是，爬虫没有集中的控制机制，但是具有更好的扩展性。存在的问题是点与点之间需要进行频繁的网络通信，用以协调整个系统的抓取任务。每个爬虫节点会把新的抽取到的URL队列分为两部分，一部分留给自己，一部分需要发送到其他爬虫节点中去。在一般情况下，每个爬虫会维护一张URL分发映射表，将不属于自己管辖的URL爬去任务按照映射表分发给其他爬虫。在此基础上，爬虫节点自身还需要负责URL的去重、过滤、存储等任务。该模式的优点是避免了主节点因为负载压力过大而导致的爬虫效率问题，所以将压力分摊到了所有的爬虫节点上，而需要注意的就是爬虫节点间的通信协调问题。

混合模式，顾名思义就是讲上述两种模式进行混合使用，既有主从模式简单易操作的特点也有点对点模式的高效率。但是这种模式的爬虫系统设计和使用起来难度更高，一般适用于大型分布式网络爬虫系统中。

综合考虑，本文采用一种改进式的主从模式进行分布式网络爬虫的整体架构。爬虫节点获取的URL和P2P模式一样分为两部分，一部分在本节点进行操作，剩余部分利用主节点进行分发。
...
本章主要介绍了本分布式网络爬虫系统的主要架构，介绍了整体的架构为一种改进的master-slaver模式的分布式网络爬虫，并且详细介绍了网络爬虫机器之间进行通信的一些格式和概念，为详细介绍网络爬虫的关键模块做好铺垫
...
##### 本章小结
本章介绍了分布式网络爬虫中的关键模块的设计，通过这些关键模块的设计对于分布式网络爬虫系统进行了进一步的剖析。主要分为：队列模块，存储模块，过滤模块，页面解析模块，下载模块等

### 第五章 实验效果及分析
##### 系统实现与配置
本文实现了一个改进式的主从结构分布式网络爬虫系统，在Unix开发环境下，该系统采用Python2.7编写，共有2000行代码，分为10个包，具有良好的代码结构和注释。为保证系统具有高度可配置型，所有的可配置项都提出了出来，放在了一个json文件中。下面的表格列出了改爬虫系统的基本参数信息：



##### 系统启动及运行结果
本文使用虚拟机来模拟分布式环境。一台Unix电脑当做主机，虚拟机为两个Ubuntu 14.04的客户端。
首先，我们在服务器端启动Redis服务器，在客户端启动MongoDB，在服务器端运行服务器程序，开启HTTP服务器。在客户端进行种子和抓取内容的正则匹配，并启动服务器端。
客户端抓取过程入下图所示：

其中【fetch】URL表示已经抓取到的URL序列，remainer参数表示服务器端队列所拥有的项目，client_count表示目前客户端队列中所拥有的项目。

服务器端LOG如下所示：
项目|解释
----|------
GET /empty | 用来检测服务器端是否为空，并且从客户端返回客户端状态
GET /pull  | 用来从服务器发送URL给客户端，客户端检测当前URL的合法性，并且加入到客户端队列中
GET /push | 客户端用来发送解析到的URL数据给服务器端，服务器端进一步处理之后分配给客户端

其中信息通过自定义HTTP头进行传递。

对于此过程中出现的由于HTTP错误导致没有正常抓取的内容，将其回放入客户端的error.log文件中，并标记处时间点和错误的HTTP状态码。方便以后进行使用：如图：


##### 本章总结
本文介绍了本爬虫系统运行时候的状态以及如何使用本爬虫系统来进行下一步操作。对爬虫系统的一些特点也做了说明。
